...
import config
from config import commandsCountMassive, commandsCountMassiveCoordinates, commandsCount, blockCount, c_n, field
...
def main():
    ...
    while True:
        ...
        if time_sinking is True:
            print(f'Проход: {sinking}. Затраченное время на обработку - {time.time() - time_start}')
        # time.sleep(0.05)
...
while len(commandsCountMassiveCoordinates) != commandsCount:
    ...
    if field[x_com][y_com] == [0, 0, 0, 0]:
        field[x_com][y_com] = [commandsCountMassive[ind_x][0], commandsCountMassive[ind_x][1],
                               commandsCountMassive[ind_x][2], 1]
